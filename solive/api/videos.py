...
from ..exts import db
from ..main.url_cate_mappings import SO_TIME
...
def get_hosts():
    ...
    if args['source'] == '全部':
        ...
        clause1 = (Video.source == args['source'])
    if args['cate'] == '全部':
        clause2 = True
    else:
        clause2 = (Video.cate == args['cate'])
    videos = Video.query.filter(db.and_(clause1, clause2)).filter(Video.latest(Video, SO_TIME)) \
...
def get_videos():
    ...
    videos = Video.query.filter((Video.cate == args['cate']) | (Video.parent_cate_name == args['cate']))\
                .filter(Video.latest(Video, SO_TIME)).order_by(Video.viewers_num.desc())[:8]
    return jsonify({
        'code': '200',
        'message': 'success',
